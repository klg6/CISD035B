...
public class ParkingLot {
    ...
    public final IntegerProperty totalCars = new SimpleIntegerProperty(0);
    ...
    //status list tracking for vehicle finder panel
    private final List<Vehicle> activeVehiclesMasterList = new ArrayList<>();

    public boolean isFull = false; //parking lot state to see if its full or not
    ...
    public void simulateParking(Vehicle v, ParkRate rate, RandomizeDuration p, int ticketNumber) {
        ...
        v.updateCellCoords(targetFloor, targetRow, targetCol);
        ...
        // --- ADD THIS LINE ---
        activeVehiclesMasterList.add(v);

        v.setCurrentStatus(Vehicle.Status.TRANSITING); //means the vehicle is moving to their cells (visually in UI)
    ...
    private void finalizeArrivalState(Vehicle v, int finalFloor, int finalCol, int finalRow, RandomizeDuration p,
        ...
        v.setCurrentStatus(Vehicle.Status.PARKED);
        ...
        stayTimer.setOnFinished(stayEvent -> {
            ...
            exitBuffer.setOnFinished(exBufferEvent -> {
                ...
                v.setCurrentStatus(Vehicle.Status.EXITING);
    ...
    private void completeCleanUp(Vehicle v, int finalFloor, int finalCol, int finalRow, ParkRate rate, double parkingStayDuration) {
        ...
        activeVehiclesMasterList.remove(v);
        Circle dot = v.getDot();
    ...
    //for the vehicle finder panel to fetch added vehicles that is currently in the lot
    public List<Vehicle> getActiveVehicles() {
        return new ArrayList<>(activeVehiclesMasterList);
    }

    //for the vehicle finder panel to fetch current vehicle cell coords
    public String getVehicleCoordinates(Vehicle target) {
        for (int f = 0; f < floors; f++) {
            for (int c = 0; c < cols; c++) {
                for (int r = 0; r < rows; r++) {
                    if (occupancy[f][c][r] == target) {
                        return String.format("%d,%d,%d", f+1, r+1, c+1);
                    }
                }
            }
        }
    ...
    public boolean isCellOccupied(int f, int r, int c) {
        // Basic bounds check to prevent errors
        if (f < 0 || f >= floors || r < 0 || r >= rows || c < 0 || c >= cols) return false;
        return occupancy[f][c][r] != null;
    }
}